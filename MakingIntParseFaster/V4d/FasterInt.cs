...
namespace MakingIntParseFaster.V4d
{
    public static class FasterInt
    {
        ...
        {
            if (s == null) ThrowArgumentNullException();
            ...
            fixed (char* sptr = s)
            {
                ...
                else if (cptr == sptr && (next = MatchChars(cptr, info.NegativeSign)) != null)
                {
                    isNegative = true;
                    cptr = next;
                    goto handleNumber;
            ...
            {
                ret = -ret;
                if (ret > 0) ThrowOverflowException();
            }
            else
            {
                if (ret < 0) ThrowOverflowException();
        ...
        {
            throw new ArgumentNullException("SR.Overflow_Int32");
        }
